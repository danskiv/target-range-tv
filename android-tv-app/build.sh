...
APP_DIR="/home/ubuntu/Github/target-range-tv/android-tv-app"
...
javac -source 1.8 -target 1.8 -d "$OBJ_DIR" -cp "$ANDROID_JAR" \
    "$APP_DIR/src/com/danskiv/targetrangetv/"*.java

echo "=== 4. Dexing classes with D8/R8 ==="
java -cp "$R8_JAR" com.android.tools.r8.D8 \
    --output "$OUT_DIR/" \
    --lib "$ANDROID_JAR" \
    "$OBJ_DIR/com/danskiv/targetrangetv/"*.class
...
apksigner sign --ks "$APP_DIR/debug.keystore" --ks-pass pass:android \
    --out "$OUT_DIR/TargetRangeTV-v1.0.apk" aligned.apk

echo "=== ✅ BUILD SUCCESS: $OUT_DIR/TargetRangeTV-v1.0.apk ==="
mkdir -p /home/ubuntu/apk-delivery
cp "$OUT_DIR/TargetRangeTV-v1.0.apk" /home/ubuntu/apk-delivery/TargetRangeTV-v1.0.apk
